...
| `app.api.enhancement_routes`      | **Enhancement APIs**               | Exposes endpoints for starting enhancement analysis, getting version history, requesting individual paragraph rewrites, and applying full document revisions. |
| `app.enhancement.classifier`      | **Paragraph Classifier**           | Automatically classifies paragraphs into issue categories based on linguistic metrics and LLM verification.                                                   |
| `app.enhancement.planner`         | **Revision Planner**               | Formulates step-by-step revision strategies, estimating priority, confidence, and expected benefit.                                                           |
| `app.enhancement.validator`       | **Revision Validator**             | Enforces academic integrity constraints (citations, numbers, formulas) on LLM-rewritten paragraphs.                                                           |
| `app.enhancement.rewriter`        | **Paragraph Rewriter**             | Coordinates LLM calls on Groq to rewrite paragraphs with an inline correction retry loop on failure.                                                          |
| `app.enhancement.diff_engine`     | **Difference Engine**              | Computes inline additions and deletions between versions, producing clean HTML highlighted diffs.                                                             |
| `app.enhancement.document_writer` | **Document Writer**                | Rewrites existing Word (.docx) documents, swapping selected paragraphs while maintaining all original styles.                                                 |
| `app.enhancement.comparison`      | **Version History Manager**        | Manages in-memory document versions, tracking transitions from initial version to final accepted revision.                                                    |
| `app.enhancement.report`          | **Enhancement Compiler**           | Compiles full enhancement analytics, compiling progress stats and final document quality metrics.                                                             |
...
## 7. Intelligent Document Revision & Originality Enhancement Engine

PlagCheck AI includes a state-of-the-art **Intelligent Document Revision & Originality Enhancement Engine**. This system acts as an interactive academic co-pilot, helping authors rewrite flagged segments to improve readability, remove accidental plagiarism patterns, and correct passive voice verbs, all while maintaining rigorous academic integrity.

### A. Paragraph Classification & Revision Planning

When a document is submitted, the system splits it into paragraphs. Each paragraph is analyzed by a hybrid pipeline:

- **Linguistic Metrics**: `WritingAnalytics` computes Flesch Reading Ease, passive voice percentages, vocabulary variety (Type-Token Ratio), and 3-gram repetitions.
- **LLM Verification**: If local metrics flag low readability, passive voice, or high similarity, a fast LLM classifies the specific issue (e.g. "Low Readability", "Needs Better Citation", "Mechanical Writing Style", "Weak Academic Tone").
- **Revision Strategy Formulation**: For any paragraph flagged for improvement, the `RevisionPlanner` generates a structured plan containing specific issues, an improvement strategy, a priority level, and an estimated quality benefit.

### B. Strict Academic Integrity Rewriting Loop

To guarantee that the AI remains a factual assistant rather than a hallucinatory writer, the `ParagraphRewriter` enforces strict constraints:

- **Attribution & Citations Preservation**: All citations (e.g. `[1]`, `(Smith, 2020)`) must be preserved in their exact positions.
- **Factual Integrity**: LaTeX equations, mathematical variables, and exact numbers cannot be modified or deleted. No new facts or unsupported references can be injected.
- **Validation and Correction Loop**: The rewritten output is validated by a `RevisionValidator` check. If a violation is detected (such as missing a citation or equation), the system runs a single-attempt retry correction loop, feeding the exact error back to the model. If it still fails, it falls back to the original text.

### C. Inline Diff & Style-Preserved Document Export

Authors can review and apply edits dynamically:

- **HTML Visual Diff**: The `DifferenceEngine` computes character-level additions and deletions using a Gestalt pattern matching approach, outputting highlighted diffs (`<ins>` and `<del>` HTML elements).
- **Style-Preserving Export**: Once revisions are accepted, the `DocumentWriter` edits the original `.docx` file in-place, swapping the paragraph text objects inside the XML structure while preserving the exact layout, margins, and headings of the original document.
